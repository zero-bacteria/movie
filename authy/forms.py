...
def ForbiddenUsers(value):
	...
	if value.lower() in forbbidden_users:
		raise ValidationError('사용할 수 없는 ID 입니다.')

def InvalidUser(value):
	if '@' in value or '+' in value or '-' in value:
		raise ValidationError('사용할수 없는 ID 입니다. 다음의 문자는 불가능합니다.: @, - , +')
...
class SignupForm(forms.ModelForm):
	...
	confirm_password = forms.CharField(widget=forms.PasswordInput(), required=True, label='비밀번호를 확인해 주세요.')
